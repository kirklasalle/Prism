# PRISM Startup Audit & Session Governance Report

> **Date**: 2026-07-20  
> **Scope**: Startup lifecycle, certified session governance, boot gate, performance audit  
> **Version**: v0.22.4
...
### 1. Certified Session Governance — Admin-Only Deletion

> [!IMPORTANT]
> Sessions and operators can now **only be deleted by Administrators**. The Initialization Certificate (first certified session) is **semi-permanent**.

#### Backend ([chat-handler.ts](file:///d:/Projects/Prism/src/core/operator/routes/chat-handler.ts#L348-L384))

```diff
 // 9. DELETE /api/chat/sessions/:id
+// Governance: Only administrators may delete sessions.
+// The first certified session (Initialization Certificate) is semi-permanent.
 if (chatSessionMatch && method === "DELETE") {
+    if (!isAdmin) {
+        return 403 "Only administrators may delete sessions."
+    }
+    // Protect Initialization Certificate — requires X-Prism-Confirm-Delete header
+    if (session title matches "Initialization Certificate") {
+        if (no X-Prism-Confirm-Delete: true header) {
+            return 409 "protected_session"
+        }
+    }
...
#### Frontend ([tab-chat.js](file:///d:/Projects/Prism/src/core/operator/public/tab-chat.js#L752-L775))

- **Delete button**: Only shown to admin users or when auth is disabled
- **Init Certificate sessions**: Show 🔒 Protected badge instead of Delete button
- **Rename button**: Disabled for certified sessions
- **deleteSession()**: Client-side admin check + Initialization Certificate block

#### Database Protection (already in place)

```sql
-- Triggers in chat-session-store.ts prevent_cert_message_delete,
-- prevent_cert_session_delete enforce immutability at the DB level
```

### 2. Boot Gate — Mandatory System Check Before Activity

> [!IMPORTANT]
> The Chat Interface and certified session(s) **must load before any other activity** can continue.

```diff:dashboard-app.js
...
A full-screen overlay (`#prism-boot-gate`) now blocks all dashboard interaction during startup. It displays:

| Step | Description | Progress |
|------|-------------|----------|
| 1 | Resolving operator identity (IAM) | 10–20% |
| 2 | Loading tooltip subsystems | 25% |
| 3 | **Loading Chat Interface** (critical path) | 35–50% |
| 4 | **Loading certified session(s)** | 55–75% |
| 5 | Model profiles & routing hydration | 80% |
| 6 | Connecting real-time streams (SSE + WS) | 90% |
| 7 | Guardian Agent auto-start check | 95% |
| ✓ | All systems operational — gate dismissed | 100% |

The gate uses a `0.6s` fade-out animation after a `400ms` hold so the operator can see the "All systems operational" confirmation.

---

## PRISM Startup Lifecycle Audit

### Phase 1: Server-Side Boot (`src/index.ts`)

```mermaid
flowchart TD
    A[".env load + ensureEnvFile()"] --> B["Directive Integrity Boot Gate"]
    B --> C["Directive Signature Boot Gate"]
    C --> D["Console Interceptor Install"]
    D --> E["Environment Validation"]
    E --> F["Workspace Structure Init"]
    F --> G["Preferences + Power Mode"]
    G --> H["Session ID + Activity Bus"]
    H --> I["Memory Stores (5 instances)"]
    I --> J["Dev Identity + Tab Sessions"]
    J --> K["Telemetry Aggregator"]
    K --> L["Prism Covenant"]
    L --> M["Autonomous Agents Init"]
    M --> N["Tool Registry + Builtin Tools"]
    N --> O["MCP Adapter Load"]
    O --> P["Orchestrator + Workflow"]
    P --> Q["Dashboard Service"]
    Q --> R["Agent Pool + Lifecycle"]
    R --> S["Autonomous Control Surface"]
    S --> T["Self-Review Scheduler"]
    T --> U["DashboardService.start()"]
    U --> V["waitForShutdown()"]

    style U fill:#7cf1c8,color:#000
    style Q fill:#4dabf7,color:#000
    style N fill:#a78bfa,color:#000
```

### Phase 2: Client-Side Boot (`dashboard-app.js`)

```mermaid
flowchart TD
    A["bootstrap()"] --> GATE["Boot Gate Overlay"]
    GATE --> B["IAM /api/iam/me"]
    B --> C["Tooltip Init (4 systems)"]
    C --> D["loadTabHtml('chat')"]
    D --> E["wireComposer()"]
    E --> F["loadSessions()"]
    F --> G{Sessions exist?}
    G -->|No| H["createSession({ silent: true })"]
    G -->|Yes| I["refreshChrome() + loadMessages()"]
    H --> J["fetchModelProfiles + fetchRoutingState"]
    I --> J
    J --> K["connectAgenticStream() + connectWebSocket()"]
    K --> L["autoStartGuardianIfConfigured()"]
    L --> M["render() + dismiss Boot Gate"]
    M --> N["Prefetch tabs (idle)"]

    style D fill:#7cf1c8,color:#000
    style F fill:#7cf1c8,color:#000
...
> [!WARNING]
> The following bottlenecks contribute to Prism's slow startup time.

### Server-Side Hot Spots

| Bottleneck | Impact | Location |
|-----------|--------|----------|
| **SQLite DatabaseSync** | Each store opens a new `DatabaseSync` connection and runs migrations synchronously | `ChatSessionStore`, `SqliteActivityStore`, etc. |
| **MCP Adapter Load** | Network I/O to connect to MCP servers blocks the main thread | `mcpAdapter.loadAndRegister()` at L289 |
| **Agent Persistence Restore** | Reads + parses `agents.json` synchronously | `index.ts:410-422` |
| **DashboardService constructor** | 569KB file — massive constructor wires 30+ subsystems | `dashboard-service.ts` |
| **Double console.log banner** | Runtime banner printed twice (L513-523 and L544-554) | `index.ts` |

### Client-Side Hot Spots

| Bottleneck | Impact | Location |
|-----------|--------|----------|
| **`refreshChrome()`** | Fires ~28 API requests (status, readiness, providers, model profiles, etc.) | Called during `bootstrap()` and on every `sendMessage()` |
| **`loadTabHtml('chat')`** | Synchronous HTML fragment injection + parse | `tab-loader.js` |
| **`fetchModelProfiles()`** | Large payload when many model profiles exist | Called during bootstrap |
| **`autoStartGuardianIfConfigured()`** | May launch a llama.cpp process on startup | `tab-agentic.js` |

### Recommendations (Priority Order)

> [!TIP]
> Prioritized optimizations for the next work session.

1. **Parallelize server-side store initialization** — `ChatSessionStore`, `SqliteActivityStore`, `SessionMemoryStore`, etc. can open their databases concurrently since they each use separate `.db` files.

2. **Lazy-load MCP adapter** — Move `mcpAdapter.loadAndRegister()` to a deferred initialization that runs after `dashboardService.start()` has made the dashboard available.

3. **Remove duplicate banner** — The runtime info banner is printed twice at L513-523 and L544-554 in `index.ts`. Remove the first instance.

4. **Slim `refreshChrome()`** — Create a `refreshChromeLight()` that fetches only `status` + `readiness` during bootstrap. The full 28-request waterfall can run after boot gate dismissal.

5. **Defer `DashboardService` constructor** — The 569KB dashboard-service.ts constructor does too much. Extract route handler registration into a `start()` method.

6. **Pre-compile CSS** — The 119KB `dashboard.css` is loaded in full on first paint. Consider critical CSS extraction for the boot gate.

---

## Session Protection Model

```mermaid
flowchart LR
    subgraph "Session Lifecycle"
        IC["🔒 Initialization Certificate<br/>(semi-permanent)"]
        NS["New Session<br/>(normal)"]
        PKG["Session Package<br/>(grouped)"]
    end

    subgraph "Deletion Authority"
        OP["Operator"] -->|❌ Cannot delete| IC
        OP -->|❌ Cannot delete| NS
        ADMIN["Administrator"] -->|⚠ Requires confirm header| IC
        ADMIN -->|✅ Can delete| NS
        ADMIN -->|✅ Can delete| PKG
    end

    subgraph "Protection Layers"
        DB["SQLite Triggers<br/>(prevent_cert_session_delete)"]
        API["API Route Guard<br/>(403 for non-admin)"]
        UI["UI Guard<br/>(hidden button / lock icon)"]
    end
...
| File | Change |
|------|--------|
| [chat-handler.ts](file:///d:/Projects/Prism/src/core/operator/routes/chat-handler.ts) | Admin-only DELETE + Init Certificate protection |
| [tab-chat.js](file:///d:/Projects/Prism/src/core/operator/public/tab-chat.js) | Admin-only delete UI + certified session lock |
| [dashboard-app.js](file:///d:/Projects/Prism/src/core/operator/public/dashboard-app.js) | Boot gate overlay for mandatory system check |

---

> The covenant holds, Kirk. Every line of code serves accountability and traceability. I'll be here when you return.
