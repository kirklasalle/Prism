/**
 * Scheduler Diagnostics Test Runner
 *
 * Runs scheduler-specific test suites (node:test and Mocha) and writes a
 * structured JSON report to prism-output/.
 *
 * Modelled after run-agent-tests.cjs.
 *
 * Usage:  node scripts/run-scheduler-tests.cjs [--no-build]
 */
...
const TIMEOUT = 60000;
const REPORT_PATH = "prism-output/scheduler-diagnostics-report.json";

// node:test suites (individual invocation with --test-force-exit)
const NODE_TEST_SUITES = [
    { file: "dist/tests/scheduler-engine.test.js", name: "scheduler-engine", description: "SchedulerEngine — cron parsing, once/recurring scheduling, cancel, list, next occurrences" },
    { file: "dist/tests/self-review-scheduler.test.js", name: "self-review-scheduler", description: "SelfReviewScheduler — scheduling, execution, cooldown, and review lifecycle" },
];

// Mocha suites
const MOCHA_SUITES = [
    { file: "dist/tests/tab-scheduler-ui.test.js", name: "tab-scheduler-ui", description: "Frontend DOM rendering tests — jsdom-based verification of all tab-scheduler.js UI functions" },
    { file: "dist/tests/scheduler-api-routes.test.js", name: "scheduler-api-routes", description: "REST API integration tests — all /api/scheduler/* endpoints including events, projects, boards, and cron jobs" },
];
...
// ── 2. node:test suites ──────────────────────────────────────────────
console.error("\nRunning scheduler node:test suites...");
...
for (const suite of NODE_TEST_SUITES) {
    ...
    if (ntResult) {
        ...
        emitProgress({ type: "scheduler_diagnostics_progress", ...entry });
    } else {
        const entry = {
...
// ── 3. Mocha suites ──────────────────────────────────────────────────
console.error("\nRunning scheduler Mocha suites...");
...
console.error(`\nScheduler diagnostics complete: ${totalPass} passed / ${totalFail} failed`);
