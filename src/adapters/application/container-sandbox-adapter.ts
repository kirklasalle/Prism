...
export class ContainerSandboxAdapter {
    ...
    private activeContainers: Map<string, { process: ChildProcess; container: Container }> = new Map();
    ...
    async startContainer(container_id: string): Promise<Container> {
        ...
        // Simulate container startup (in real scenario, would use docker/containerd/etc)
        const mockProc = spawn("sh", ["-c", "sleep infinity"], {
    ...
    async execInContainer(
        ...
        const { process: containerProc, container } = containerEntry;
        ...
        return new Promise((resolve, reject) => {
            containerProc.stdin?.write(command + "\n");
            ...
            containerProc.stdout?.on("data", stdoutListener);
            containerProc.stderr?.on("data", stderrListener);

            // Exit code detection
            const exitCodeRegex = /\$\?=(\d+)/;
            const checkExitCode = () => {
                containerProc.stdin?.write("echo $?=\\$?\n");
                ...
                const onExitData = (data: Buffer) => {
                    ...
                    if (match) {
                        ...
                        containerProc.stdout?.removeListener("data", stdoutListener);
                        containerProc.stdout?.removeListener("data", onExitData);
                        containerProc.stderr?.removeListener("data", stderrListener);
                ...
                containerProc.stdout?.once("data", onExitData);
            };
    ...
    async snapshotContainer(
        ...
        const snapshot: ContainerSnapshot = {
            ...
            snapshot_name,
            description,
            snapshot_size_mb: Math.random() * 1000, // Simulated size
    ...
    async revertContainer(
        ...
        // Restart container (simulated)
        const containerEntry = this.activeContainers.get(container_id);
        if (containerEntry) {
            const { process: containerProc } = containerEntry;
            containerProc.kill();
        }
    ...
    private async monitorResourceUsage(container_id: string): Promise<void> {
        ...
        // In real scenario, would poll /proc/[pid]/stat and /proc/[pid]/status
