...
import { type SSHPInterceptor, SSHP_SENSITIVE_SELECTORS } from "../../core/operator/sshp-interceptor.js";
...
const BROWSER_GOVERNANCE: GovernanceSchema = {
    actions: {
        ...
        drag_and_drop: { minimumRisk: "medium", mutating: true, rollbackRequired: false },
            ...
            switch (action) {
                ...
                case "launch_session": {
                    ...
                    return {
                        ...
                        sideEffects: [
                            {
                                type: "process",
                                description: `Browser session launched: ${session.id}${profileId ? ` (profile: ${profileId})` : ""}`,
                                mutating: true,
                                reversible: true,
                            },
                        ],
                ...
                case "close_session": {
                    ...
                    return {
                        ...
                        sideEffects: [
                            {
                                type: "process",
                                description: `Browser session closed: ${sessionId}`,
                                mutating: true,
                                reversible: false,
                            },
                        ],
                ...
                case "navigate": {
                    ...
                    if (this.sshpInterceptor) {
                        ...
                        if (!audit.allowed) {
                            return {
                                ok: false,
                                output: {
                                    error: `SSHP Covenant block: ${audit.reason || "Action violates Sacred Covenant articles"}`,
                                },
                            };
                        }
                    }
                    ...
                    return {
                        ...
                        sideEffects: [
                            { type: "network", description: `Navigated to ${url}`, mutating: false, reversible: true },
                        ],
                    };
                }
                ...
                case "click": {
                    ...
                    return {
                        ...
                        sideEffects: [
                            { type: "process", description: `Clicked: ${selector}`, mutating: true, reversible: false },
                        ],
                    };
                }
                ...
                case "screenshot": {
                    ...
                    if (this.sshpInterceptor && this.sshpInterceptor.isEnabled()) {
                        ...
                        if (handles && handles.page) {
                            const rects = await handles.page
                                .evaluate(
                                    (selectors: string[]) => {
                    ...
                    return {
                        ...
                        sideEffects: [
                            { type: "file", description: "Screenshot captured", mutating: false, reversible: true },
                        ],
                    };
                }
                ...
                case "evaluate": {
                    ...
                    return {
                        ...
                        sideEffects: [
                            {
                                type: "process",
                                description: `Evaluated JS expression (${expression.length} chars)`,
                                mutating: true,
                                reversible: false,
                            },
                        ],
                ...
                case "select_option": {
                    ...
                    const sel = String(request.args.selector ?? "");
                    if (!sel) return { ok: false, output: { error: "selector is required." } };
                    const vals = String(request.args.values ?? "")
                        .split(",")
                        .map((v) => v.trim())
                        .filter(Boolean);
                    ...
                    return {
                        ok: true,
                        output: { sessionId, selector: sel, selected } as unknown as Record<string, unknown>,
                    };
                }
                case "scroll": {
                    if (!sessionId) return { ok: false, output: { error: "sessionId is required." } };
                ...
                case "get_links": {
                    ...
                    return {
                        ok: true,
                        output: { sessionId, links, count: links.length } as unknown as Record<string, unknown>,
                    };
                }
                ...
                case "get_cookies": {
                    ...
                    return {
                        ok: true,
                        output: { sessionId, cookies, count: cookies.length } as unknown as Record<string, unknown>,
                    };
                }
                case "set_cookie": {
                    if (!sessionId) return { ok: false, output: { error: "sessionId is required." } };
                    ...
                    try {
                        ...
                        return { ok: false, output: { error: "cookie must be valid JSON." } };
                    }
                    await this.manager.setCookie(sessionId, cookieObj);
                    return {
                        ok: true,
                        output: { sessionId, cookieSet: true },
                        sideEffects: [{ type: "process", description: "Cookie set", mutating: true, reversible: true }],
                    };
                }
                case "clear_cookies": {
                    if (!sessionId) return { ok: false, output: { error: "sessionId is required." } };
                    ...
                    return {
                        ok: true,
                        output: { sessionId, cookiesCleared: true },
                        sideEffects: [
                            { type: "process", description: "Cookies cleared", mutating: true, reversible: false },
                        ],
                    };
                }
                ...
                case "list_profiles": {
                    ...
                    return {
                        ok: true,
                        output: { profiles, count: profiles.length } as unknown as Record<string, unknown>,
                    };
                }
                case "create_profile": {
                    const email = String(request.args.email ?? "")
                        .toLowerCase()
                        .trim();
                    ...
                    const profile = this.profileManager.createProfile({
                        ...
                        executionProfileSegment: segment,
                    });
                    return {
                        ok: true,
                        ...
                        sideEffects: [
                            {
                                type: "file",
                                description: `Browser profile created: ${profile.profileId}`,
                                mutating: true,
                                reversible: true,
                            },
                        ],
                ...
                case "delete_profile": {
                    ...
                    return {
                        ok: true,
                        output: { profileId: pid, deleted },
                        sideEffects: [
                            {
                                type: "file",
                                description: `Browser profile deleted: ${pid}`,
                                mutating: true,
                                reversible: false,
                            },
                        ],
