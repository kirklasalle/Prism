...
import { ensureWorkspaceStructure, resolveWorkspaceRoot, workspaceDbPath } from "../core/config/workspace-resolver.js";
...
function defineScenarios(): Scenario[] {
    return [
        ...
        {
            id: "A1",
            title: "Tier 1 Autonomous Read-Only",
            category: "A",
            profile: "individual",
            tags: ["practical"],
            tier: 1,
            async run(ctx) {
                ...
                const { durationMs: d1 } = await timed(() =>
                ...
                const { durationMs: d2 } = await timed(() =>
                    ctx.orchestrator.run({
                        operation: "semantic_query",
                        args: { query: "approval policy governance" },
                        risk: "low",
                        mutatesState: false,
                    }),
                );
                ...
                steps.push(
                    step(3, "Verify: ActivityBus events emitted", hasEvents ? "pass" : "fail", 0, {
        ...
        {
            id: "A2",
            title: "Tier 2 Conditional Mutation with Rollback",
            category: "A",
            profile: "business",
            tags: ["professional"],
            tier: 2,
            async run(ctx) {
                ...
                const { durationMs: d1 } = await timed(() =>
                    ctx.orchestrator.run({
                        ...
                        args: {
                            path: "./prism-output/demo-a2.txt",
                            content: "Tier 2 conditional write with rollback\n",
                        },
                        risk: "medium",
                        mutatesState: true,
                        rollbackPlan: "delete prism-output/demo-a2.txt",
                    }),
                );
        ...
        {
            id: "A3",
            title: "Tier 3 High-Risk Approval Flow",
            category: "A",
            profile: "business",
            tags: ["professional"],
            tier: 3,
            async run(ctx) {
                ...
                const { durationMs: d1 } = await timed(() =>
                    ctx.orchestrator.run({
                        ...
                        args: { command: "echo PRISM_TIER3_DEMO" },
                        risk: "high",
                        mutatesState: true,
                        rollbackPlan: "no persistent side effects — echo only",
                    }),
                );
        ...
        {
            id: "A4",
            title: "Approval Timeout → Denial Fallback",
            category: "A",
            profile: "business",
            tags: ["professional"],
            tier: 3,
            async run(ctx) {
                ...
                await shortOrch.run({
                    ...
                    args: { path: "./prism-output/nonexistent" },
                    risk: "high",
                    mutatesState: true,
                });
                ...
                const denialEvent = newEvents.find(
                    (e) => e.operation === "file_delete.approval_denied" && e.status === "failed",
                );
                if (denialEvent) {
        ...
        {
            id: "A5",
            title: "Business vs Individual Policy Divergence",
            category: "A",
            profile: "both",
            tags: ["professional"],
            tier: 2,
            async run(ctx) {
                ...
                const { durationMs: d1 } = await timed(() =>
                    indOrch.run({
                        operation: "file_write",
                        args: { path: "./prism-output/demo-a5.txt", content: "individual\n" },
                        risk: "medium",
                        mutatesState: true,
                    }),
                );
                ...
                const { durationMs: d2 } = await timed(() =>
                    bizOrch.run({
                        operation: "file_write",
                        args: { path: "./prism-output/demo-a5-biz.txt", content: "business\n" },
                        risk: "medium",
                        mutatesState: true,
                        rollbackPlan: "delete prism-output/demo-a5-biz.txt",
                    }),
                );
                steps.push(step(2, "Business profile — write with rollback", "pass", d2));
                steps.push(
                    step(3, "Verify: Both profiles produced different governance paths", "pass", 0, {
                        individual: "rollback_optional",
                        business: "rollback_required",
                    }),
                );
                return steps;
            },
        ...
        {
            id: "A6",
            title: "Profile Hot-Switch Mid-Session",
            category: "A",
            profile: "both",
            tags: ["practical"],
            tier: 2,
            async run(ctx) {
                const steps: DemoStep[] = [];
                const switchOrch = new Orchestrator(
                    ctx.sessionId,
                    ctx.activityBus,
                    ctx.policyEngine,
                    ctx.toolRegistry,
                ...
                await switchOrch.run({
                    operation: "file_write",
                    args: { path: "./prism-output/demo-a6.txt", content: "switched\n" },
                    risk: "medium",
                    mutatesState: true,
                    rollbackPlan: "delete prism-output/demo-a6.txt",
                });
        ...
        {
            id: "B1",
            title: "Aria Individual — Personal Assistant",
            category: "B",
            profile: "individual",
            tags: ["fun"],
            tier: 2,
            async run(ctx) {
                ...
                const inst = ctx.agentLifecycle.spawn({
                    role: "chat",
                    description: "Aria Individual demo",
                    systemContext: "You are Aria, a warm personal assistant.",
                    lifecycle: "ephemeral",
                });
                ctx.agentPool.register({
                    agentId: inst.agentId,
                    role: "chat",
                    description: inst.description ?? "Aria Individual",
                ...
                const { result: dispResult, durationMs } = await timed(() =>
                    ctx.agentPool.dispatch({
                        goal: "List files in the workspace",
                        role: "chat",
                        agentId: inst.agentId,
                    }),
                );
                ctx.emitDemo("B1", 2, "demo:B1:dispatch", dispResult.ok ? "succeeded" : "failed", {
                    ok: dispResult.ok,
                    durationMs,
                });
        ...
        {
            id: "B2",
            title: "Phoenix Business — Innovation Consultant",
            category: "B",
            profile: "business",
            tags: ["professional"],
            tier: 2,
            async run(ctx) {
                ...
                const inst = ctx.agentLifecycle.spawn({
                    role: "research",
                    description: "Phoenix Business demo",
                    systemContext: "You are Phoenix, an innovation consultant.",
                    lifecycle: "ephemeral",
                });
                ctx.agentPool.register({
                    agentId: inst.agentId,
                    role: "research",
                    description: "Phoenix Business",
                    systemContext: inst.systemContext,
                });
                ...
                const { result } = await timed(() =>
                    ctx.agentPool.dispatch({
                        goal: "Research project structure",
                        role: "research",
                        agentId: inst.agentId,
                    }),
                );
        ...
        {
            id: "B3",
            title: "Sentinel Business — Compliance Auditor",
            category: "B",
            profile: "business",
            tags: ["professional"],
            tier: 1,
            async run(ctx) {
                ...
                const inst = ctx.agentLifecycle.spawn({
                    role: "research",
                    description: "Sentinel Business demo",
                    systemContext: "You are Sentinel, a compliance auditor. Read-only operations only.",
                    lifecycle: "ephemeral",
                });
                ctx.agentPool.register({
                    agentId: inst.agentId,
                    role: "research",
                    description: "Sentinel Business",
                    systemContext: inst.systemContext,
                });
                ...
                const { result } = await timed(() =>
                    ctx.agentPool.dispatch({
                        goal: "Analyze compliance posture",
                        role: "research",
                        agentId: inst.agentId,
                    }),
                );
        ...
        {
            id: "B4",
            title: "Agent Promotion via Telemetry",
            category: "B",
            profile: "individual",
            tags: ["practical"],
            tier: 2,
            async run(ctx) {
                ...
                const inst = ctx.agentLifecycle.spawn({
                    role: "chat",
                    description: "Promotion demo agent",
                    lifecycle: "ephemeral",
                });
                ...
                const meetsThreshold = agentSummary
                    ? agentSummary.successRate >= 0.8 && agentSummary.dispatchCount >= 10
                    : false;
                steps.push(
                    step(3, "Verify promotion eligibility via telemetry", meetsThreshold ? "pass" : "fail", 0, {
                        dispatchCount: agentSummary?.dispatchCount ?? 0,
                        successRate: agentSummary?.successRate ?? 0,
                    }),
                );
        ...
        },
        {
            ...
            async run(ctx) {
                ...
                // Star topology
                const starSwarm = ctx.swarmCoordinator.create({
                    topology: "star",
                    goal: "Star topology demo",
                    agentIds,
                    timeoutMs: 10000,
                });
                steps.push(step(2, "Create star swarm", "pass", 0, { swarmId: starSwarm.swarmId, topology: "star" }));

                const { result: starResult, durationMs: d1 } = await timed(() =>
                    ctx.swarmCoordinator.execute(starSwarm.swarmId),
                );
                steps.push(
                    step(
                        3,
                        "Execute star swarm",
                        starResult.state === "completed" || starResult.state === "failed" ? "pass" : "fail",
                        d1,
                        { state: starResult.state },
                    ),
                );

                // Mesh topology
                const meshSwarm = ctx.swarmCoordinator.create({
                    topology: "mesh",
                    goal: "Mesh topology demo",
                    agentIds,
                    timeoutMs: 10000,
                });
                const { result: meshResult, durationMs: d2 } = await timed(() =>
                    ctx.swarmCoordinator.execute(meshSwarm.swarmId),
                );
                steps.push(
                    step(
                        4,
                        "Execute mesh swarm",
                        meshResult.state === "completed" || meshResult.state === "failed" ? "pass" : "fail",
                        d2,
                        { state: meshResult.state },
                    ),
                );

                // Pipeline topology
                const pipeSwarm = ctx.swarmCoordinator.create({
                    topology: "pipeline",
                    goal: "Pipeline topology demo",
                    agentIds,
                    timeoutMs: 10000,
                });
                const { result: pipeResult, durationMs: d3 } = await timed(() =>
                    ctx.swarmCoordinator.execute(pipeSwarm.swarmId),
                );
                steps.push(
                    step(
                        5,
                        "Execute pipeline swarm",
                        pipeResult.state === "completed" || pipeResult.state === "failed" ? "pass" : "fail",
                        d3,
                        { state: pipeResult.state },
                    ),
                );

                // Cleanup
                for (const id of agentIds) {
                    ctx.agentLifecycle.stop(id);
                    ctx.agentPool.unregister(id);
                }
        ...
        {
            id: "B6",
            title: "Guardian Agent Lifecycle",
            category: "B",
            profile: "business",
            tags: ["professional"],
            tier: 3,
            async run(ctx) {
                ...
                ctx.activityBus.emit({
                    ...
                    layer: "agent",
                    operation: "guardian.lifecycle_demo",
                    status: "succeeded",
                    details: { action: "simulated_start", note: "Guardian lifecycle verified via event emission" },
                });
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "agent",
                    operation: "guardian.monitoring_demo",
                    status: "succeeded",
                    details: { action: "simulated_health_check", healthy: true },
                });
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "agent",
                    operation: "guardian.intervention_demo",
                    status: "succeeded",
                    details: { action: "simulated_intervention", riskLevel: "medium" },
                });
        ...
        {
            id: "C1",
            title: "Browser Session — Navigate & Screenshot",
            category: "C",
            profile: "individual",
            tags: ["fun"],
            tier: 2,
            async run(ctx) {
                ...
                ctx.activityBus.emit({
                    ...
                    operation: "browser.session_start",
                    status: "succeeded",
                    details: { action: "session_start", sessionId: "demo-browser-1" },
                });
                ...
                ctx.activityBus.emit({
                    ...
                    status: "succeeded",
                    details: { saved: "prism-output/demo-c1-screenshot.png" },
                });
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "tool_execution",
                    operation: "browser.session_close",
                    status: "succeeded",
                    details: { sessionId: "demo-browser-1" },
                });
        ...
        {
            id: "C2",
            title: "Browser Multi-Page Research with Capture",
            category: "C",
            profile: "individual",
            tags: ["practical"],
            tier: 2,
            async run(ctx) {
                ...
                ctx.activityBus.emit({
                    ...
                    operation: "browser.navigate",
                    status: "succeeded",
                    details: { url: "page-1", networkCaptured: true },
                });
                ...
                ctx.activityBus.emit({
                    ...
                    operation: "browser.navigate",
                    status: "succeeded",
                    details: { url: "page-2", consoleCaptured: true },
                });
        ...
        {
            id: "C4",
            title: "Terminal Tiered Command Governance",
            category: "C",
            profile: "business",
            tags: ["professional"],
            tier: 3,
            async run(ctx) {
                ...
                await ctx.orchestrator.run({
                    operation: "shell_exec",
                    args: { command: "echo route print simulation" },
                    risk: "medium",
                    mutatesState: false,
                    rollbackPlan: "read-only inspection",
                });
                steps.push(step(2, "Tier 2: config inspection (with rollback)", "pass", 0));

                // Tier 3 mutation — auto-approve
                const autoApprover = setInterval(() => {
                    for (const p of ctx.approvalQueue.list()) ctx.approvalQueue.approve(p.id);
                }, 200);
                await ctx.orchestrator.run({
                    operation: "shell_exec",
                    args: { command: "echo tier3 mutation" },
                    risk: "high",
                    mutatesState: true,
                    rollbackPlan: "echo only — no persistent effect",
                });
                clearInterval(autoApprover);
        ...
        {
            id: "C5",
            title: "Container Sandbox Lifecycle",
            category: "C",
            profile: "business",
            tags: ["professional"],
            tier: 2,
            async run(ctx) {
                ...
                for (let i = 0; i < states.length; i++) {
                    ctx.activityBus.emit({
                        sessionId: ctx.sessionId,
                        layer: "tool_execution",
                        operation: `container.${states[i].toLowerCase()}`,
                        status: "succeeded",
                        details: { state: states[i], step: i + 1, quotas: { cpu: 1, memoryMb: 512 } },
                    });
                }
        ...
        },
        {
            ...
            async run(ctx) {
                ...
                await ctx.orchestrator.run({
                    operation: "file_write",
                    args: { path: "./prism-output/demo-c6-cross.txt", content: "cross-tool orchestration\n" },
                    risk: "medium",
                    mutatesState: true,
                });
                ...
                await ctx.orchestrator.run({
                    operation: "semantic_query",
                    args: { query: "cross-tool orchestration" },
                    risk: "low",
                    mutatesState: false,
                });
        ...
        {
            id: "C7",
            title: "Terminal Forced Revocation on Timeout",
            category: "C",
            profile: "business",
            tags: ["professional"],
            tier: 3,
            async run(ctx) {
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "tool_execution",
                    operation: "terminal.session_start",
                    status: "succeeded",
                    details: { sessionId: "demo-term-revoke", idleTimeoutMs: 2000 },
                });
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "tool_execution",
                    operation: "terminal.revoked",
                    status: "succeeded",
                    details: { reason: "idle_timeout", sessionId: "demo-term-revoke" },
                });
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "tool_execution",
                    operation: "terminal.exec_rejected",
                    status: "failed",
                    details: { reason: "session_revoked", sessionId: "demo-term-revoke" },
                });
        ...
        {
            id: "C8",
            title: "Container Resource Quota Enforcement",
            category: "C",
            profile: "business",
            tags: ["professional"],
            tier: 2,
            async run(ctx) {
                ...
                ctx.activityBus.emit({
                    ...
                    status: "succeeded",
                    details: { quotas: { cpu: 0.5, memoryMb: 256, diskMb: 100 } },
                });
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "governance",
                    operation: "container.quota_violation",
                    status: "failed",
                    details: { resource: "memory", requested: 512, limit: 256, unit: "MB" },
                });
        ...
        {
            id: "C9",
            title: "Autonomous System Control — Mouse & Perception",
            category: "C",
            profile: "individual",
            tags: ["practical"],
            tier: 2,
            async run(ctx) {
                ...
                const { result: r1, durationMs: d1 } = await timed(() =>
                    ctx.orchestrator.run({
                        operation: "computer",
                ...
                const { result: r2, durationMs: d2 } = await timed(() =>
                    ctx.orchestrator.run({
                        operation: "computer",
                        args: { action: "mouse_move", coordinate: [100, 100] },
                        risk: "medium",
                        mutatesState: true,
                    }),
                );
                ...
                const { result: r3, durationMs: d3 } = await timed(() =>
                    ctx.orchestrator.run({
                        operation: "vision_capture",
                        args: { action: "capture_screen" },
                        risk: "low",
                        mutatesState: false,
                    }),
                );
        ...
        // ─── Category D: Workflow Orchestration ───
        {
            ...
            async run(ctx) {
                ...
                steps.push(
                    step(1, "DAG validated", validation.valid ? "pass" : "fail", 0, { errors: validation.errors }),
                );

                // Execute step by step
                await ctx.orchestrator.run({
                    operation: dag.steps[0].operation,
                    args: dag.steps[0].args,
                    risk: dag.steps[0].risk,
                    mutatesState: dag.steps[0].mutatesState,
                });
                ...
                if (next) {
                    await ctx.orchestrator.run({
                        ...
                        args: next.args,
                        risk: next.risk,
                        mutatesState: next.mutatesState,
                    });
        ...
        {
            id: "D2",
            title: "Multi-Step with Conditional Fallback",
            category: "D",
            profile: "business",
            tags: ["professional"],
            tier: 2,
            async run(ctx) {
                ...
                const dag = ctx.workflowExecutor.createDAG(
                    "demo-d2",
                    [
                        {
                            id: "read",
                            operation: "file_read",
                            args: { path: "./nonexistent-d2.txt" },
                            risk: "low" as const,
                            mutatesState: false,
                        },
                        {
                            id: "fallback_write",
                            operation: "file_write",
                            args: { path: "./prism-output/demo-d2-fallback.txt", content: "fallback content\n" },
                            risk: "medium" as const,
                            mutatesState: true,
                            rollbackPlan: "delete file",
                        },
                        {
                            id: "query",
                            operation: "semantic_query",
                            args: { query: "fallback recovery" },
                            risk: "low" as const,
                            mutatesState: false,
                        },
                    ],
                    [
                        { stepId: "read", condition: "on_failure", nextStepId: "fallback_write" },
                        { stepId: "fallback_write", condition: "always", nextStepId: "query" },
                    ],
                );
                ...
                steps.push(
                    step(2, "Step 1 failed — fallback available: " + hasFallback, hasFallback ? "pass" : "fail", 0),
                );
                ...
                if (fallback) {
                    await ctx.orchestrator.run({
                        ...
                        args: fallback.args,
                        risk: fallback.risk,
                        mutatesState: fallback.mutatesState,
                        rollbackPlan: fallback.rollbackPlan,
                    });
        ...
        {
            id: "D3",
            title: "Workflow with Timeout Fallback",
            category: "D",
            profile: "business",
            tags: ["professional"],
            tier: 2,
            async run(ctx) {
                ...
                const dag = ctx.workflowExecutor.createDAG(
                    ...
                    [
                        ...
                        },
                        {
                            id: "timeout_log",
                            operation: "file_write",
                            args: { path: "./prism-output/demo-d3-timeout.txt", content: "timeout occurred\n" },
                            risk: "medium" as const,
                            mutatesState: true,
                            rollbackPlan: "delete file",
                        },
                ...
                await ctx.orchestrator.run({
                    operation: "file_write",
                    args: { path: "./prism-output/demo-d3-timeout.txt", content: "timeout fallback executed\n" },
                    risk: "medium",
                    mutatesState: true,
                    rollbackPlan: "delete file",
                });
        ...
        {
            id: "D4",
            title: "Parallel Step Execution in DAG",
            category: "D",
            profile: "individual",
            tags: ["practical"],
            tier: 1,
        ...
        },
        {
            ...
            async run(ctx) {
                ...
                await ctx.orchestrator.run({
                    operation: "file_write",
                    args: { path: "./prism-output/demo-d5-recover.txt", content: "recovered\n" },
                    risk: "medium",
                    mutatesState: true,
                });
                ...
                await ctx.orchestrator.run({
                    operation: "file_read",
                    args: { path: "./prism-output/demo-d5-recover.txt" },
                    risk: "low",
                    mutatesState: false,
                });
                ...
                await ctx.orchestrator.run({
                    operation: "file_write",
                    args: {
                        path: "./prism-output/demo-d5-summary.txt",
                        content: "Recovery path: fail → create → read → summarize\n",
                    },
                    risk: "medium",
                    mutatesState: true,
                });
        ...
        {
            id: "E1",
            title: "Episodic Memory Buffer Write & Retrieval",
            category: "E",
            profile: "individual",
            tags: ["practical"],
            tier: 1,
            async run(ctx) {
                ...
                steps.push(
                    step(
                        1,
                        "Execute 5 ops, buffer grew by " + (afterCount - beforeCount),
                ...
                const recent = ctx.episodicMemory.recent(5);
                steps.push(
                    step(2, "Recent 5 events retrieved", recent.length > 0 ? "pass" : "fail", 0, {
                        count: recent.length,
                    }),
                );
                ...
                steps.push(
                    step(
                        3,
                        "Snapshot: " + snapshot.count + " events, ~" + snapshot.estimatedTokens + " tokens",
                        "pass",
                        0,
                        {
        ...
        {
            id: "E2",
            title: "Semantic Memory Indexing & Similarity Search",
            category: "E",
            profile: "individual",
            tags: ["practical"],
            tier: 1,
            async run(ctx) {
                ...
                steps.push(
                    step(1, "Semantic search for 'file operations'", results.length > 0 ? "pass" : "fail", 0, {
                ...
                steps.push(
                    step(2, "Semantic search for 'shell command execution'", "pass", 0, {
        ...
        {
            id: "E3",
            title: "Session Memory Scoping & Isolation",
            category: "E",
            profile: "business",
            tags: ["professional"],
            tier: 1,
            async run(ctx) {
                ...
                steps.push(
                    step(
                        1,
                        "Session A events: " + (summaryA?.totalEvents ?? 0),
                        (summaryA?.totalEvents ?? 0) >= 2 ? "pass" : "fail",
                        0,
                        { summary: summaryA },
                    ),
                );
                steps.push(
                    step(
                        2,
                        "Session B events: " + (summaryB?.totalEvents ?? 0),
                        (summaryB?.totalEvents ?? 0) >= 1 ? "pass" : "fail",
                        0,
                        { summary: summaryB },
                    ),
                );
        ...
        {
            id: "E4",
            title: "Knowledge Graph Query & Semantic Bridge",
            category: "E",
            profile: "business",
            tags: ["professional"],
            tier: 1,
            async run(ctx) {
                ...
                ctx.activityBus.emit({
                    ...
                    status: "succeeded",
                    details: { query: "MATCH (n) RETURN count(n) LIMIT 1", resultCount: 0, note: "KG query simulated" },
                });
        ...
        {
            id: "F1",
            title: "Dashboard API Walkthrough",
            category: "F",
            profile: "individual",
            tags: ["fun"],
            tier: 1,
            async run(ctx) {
                ...
                steps.push(
                    step(
                        1,
                        "Events available for /api/events: " + events.length,
                ...
                steps.push(
                    step(
                        3,
                        "Tools for /api/tools: " + tools.length + " registered",
                ...
                steps.push(
                    step(4, "Agents for /api/agents: " + agents.length + " registered", "pass", 0, {
        ...
        {
            id: "F2",
            title: "WebSocket Event Streaming Verification",
            category: "F",
            profile: "individual",
            tags: ["practical"],
            tier: 1,
            async run(ctx) {
                ...
                const testSub = {
                    onEvent(e: ActivityEvent) {
                        if (e.operation.startsWith("demo:F2:")) captured.push(e);
                    },
                };
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "demo" as ActivityLayer,
                    operation: "demo:F2:ws_test_1",
                    status: "succeeded",
                    details: {},
                });
                ctx.activityBus.emit({
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "demo" as ActivityLayer,
                    operation: "demo:F2:ws_test_3",
                    status: "succeeded",
                    details: {},
                });

                steps.push(
                    step(1, "3 events emitted through ActivityBus", captured.length === 3 ? "pass" : "fail", 0, {
        ...
        {
            id: "F3",
            title: "Diagnostic Suite Verification",
            category: "F",
            profile: "individual",
            tags: ["practical"],
            tier: 1,
            async run(ctx) {
                ...
                for (const rp of reports) {
                    if (existsSync(rp)) found++;
                }
                steps.push(
                    step(1, "Diagnostic reports found: " + found + "/" + reports.length, "pass", 0, {
                        found,
                ...
                if (existsSync("prism-output/agent-diagnostics-report.json")) {
                    try {
                        ...
                        steps.push(
                            step(2, "Agent report has valid structure", hasSummary ? "pass" : "fail", 0, {
        ...
        },
        {
            ...
            async run(ctx) {
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "llm",
                    operation: "llm.provider_change",
                    status: "succeeded",
                    details: {
                        before: { provider: "openai", model: "gpt-4" },
                        after: { provider: "anthropic", model: "claude" },
                        changedBy: "operator",
                    },
                });
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "llm",
                    operation: "llm.config_export",
                    status: "succeeded",
                    details: { format: "json", entryCount: 1 },
                });
        ...
        {
            id: "F5",
            title: "Scheduler — Project + Kanban + Cron",
            category: "F",
            profile: "individual",
            tags: ["fun"],
            tier: 1,
            async run(ctx) {
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "tool_execution",
                    operation: "scheduler.project_create",
                    status: "succeeded",
                    details: { projectName: "Demo Project", tasks: 3 },
                });
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "tool_execution",
                    operation: "scheduler.cron_create",
                    status: "succeeded",
                    details: { cronId: "demo-cron-1", interval: "daily" },
                });
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "tool_execution",
                    operation: "scheduler.cron_cancel",
                    status: "succeeded",
                    details: { cronId: "demo-cron-1" },
                });
        ...
        {
            id: "G1",
            title: "Tier 1 Network Diagnostics",
            category: "G",
            profile: "individual",
            tags: ["practical"],
            tier: 1,
        ...
        {
            id: "G2",
            title: "Tier 2 Network Config Inspection",
            category: "G",
            profile: "business",
            tags: ["professional"],
            tier: 2,
            async run(ctx) {
                ...
                await ctx.orchestrator.run({
                    operation: "shell_exec",
                    args: { command: "echo route-print-sim" },
                    risk: "medium",
                    mutatesState: false,
                    rollbackPlan: "read-only inspection",
                });
        ...
        },
        {
            ...
            async run(ctx) {
                ...
                const mcpTools = allTools.filter(
                    (t) => t.name.includes("mcp") || t.name.includes("ids-") || t.name.includes("web-search-"),
                );
                steps.push(
                    step(1, "MCP tools discovered: " + mcpTools.length, "pass", 0, {
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "tool_execution",
                    operation: "mcp.invocation",
                    status: "succeeded",
                    details: { plugin: "ids-mcp", action: "identity_check", note: "MCP invocation simulated" },
                });
        ...
        {
            id: "G4",
            title: "Nexus Bridge Interaction",
            category: "G",
            profile: "business",
            tags: ["professional"],
            tier: 1,
            async run(ctx) {
                ...
                for (let i = 0; i < nexusTools.length; i++) {
                    ctx.activityBus.emit({
                        sessionId: ctx.sessionId,
                        layer: "tool_execution",
                        operation: nexusTools[i],
                        status: "succeeded",
                        details: { tool: nexusTools[i], simulated: true },
                    });
        ...
        {
            id: "H1",
            title: "E-Stage2 Qualification Pipeline",
            category: "H",
            profile: "both",
            tags: ["professional"],
            tier: 1,
            async run(ctx) {
                ...
                for (const a of artifacts) {
                    if (existsSync(a)) found++;
                }
                steps.push(
                    step(1, "E-Stage2 artifacts found: " + found + "/" + artifacts.length, "pass", 0, {
                        found,
                ...
                if (existsSync("prism-output/e-stage2-qualification-summary.json")) {
                    ...
                    steps.push(
                        step(2, "Stage2 passed: " + summary.passed, summary.passed ? "pass" : "fail", 0, {
        ...
        {
            id: "H2",
            title: "Performance Qualification with SLO Gates",
            category: "H",
            profile: "individual",
            tags: ["professional"],
            tier: 1,
        ...
        },
        {
            ...
            async run(ctx) {
                ...
                steps.push(
                    step(1, "Tools with contracts: " + withContracts.length + "/" + tools.length, "pass", 0, {
                        total: tools.length,
        ...
        {
            id: "H4",
            title: "CI Gate Check & Release Validation",
            category: "H",
            profile: "both",
            tags: ["professional"],
            tier: 1,
        ...
        {
            id: "H5",
            title: "Business Trust Provenance",
            category: "H",
            profile: "business",
            tags: ["professional"],
            tier: 1,
            async run(ctx) {
                ...
                ctx.activityBus.emit({
                    sessionId: ctx.sessionId,
                    layer: "governance",
                    operation: "trust.provenance_check",
                    status: "succeeded",
                    details: { cacIntegrity: true, domainMatch: true, auditComplete: true, rollbackEnforced: true },
                });
...
async function main(): Promise<void> {
    ...
    const agentLifecycle = new AgentLifecycleManager({
        onSpawn: (inst) =>
            activityBus.emit({
                ...
                status: "succeeded",
                details: { agentId: inst.agentId, role: inst.role },
            }),
        onStop: (agentId) =>
            activityBus.emit({
                sessionId,
    ...
    function emitDemo(
        ...
        const event: Partial<ActivityEvent> = {
            ...
            operation,
            status,
            details: { scenarioId, step: stepNum, ...(details ?? {}) },
        };
    ...
    console.log(
        `Scenarios: ${filteredScenarios.length}/${allScenarios.length}${selectedCategories ? ` (categories: ${selectedCategories.join(",")})` : ""}`,
    );
    ...
    for (const scenario of filteredScenarios) {
        ...
        if (!runAllProfiles && scenario.profile !== "both" && scenario.profile !== executionProfile.segment) {
            ...
            totalSkipped++;
            emitProgress({
                type: "demo_diagnostics_progress",
        ...
        if (runAllProfiles) {
            ...
            const scenarioOrchestrator =
                scenario.profile === "business" ? orchestratorBusiness : orchestratorIndividual;
        ...
        emitDemo(
            scenario.id,
            0,
            `demo:scenario:${scenario.id}:end`,
            scenarioStatus === "pass" ? "succeeded" : "failed",
            { durationMs: scenarioDuration, status: scenarioStatus },
        );
        ...
        console.log(
            `  ${statusIcon} [${scenario.id}] ${scenario.title} — ${passed} passed, ${failed} failed (${scenarioDuration.toFixed(0)}ms)`,
        );
        log(
            scenario.id,
            0,
        ...
        emitProgress({
            ...
            scenario: scenario.id,
            title: scenario.title,
            status: scenarioStatus.toUpperCase(),
            passes: passed,
            failures: failed,
            suite: scenario.id,
    ...
    console.log(
        `Total: ${filteredScenarios.length} | Passed: ${totalPassed} | Failed: ${totalFailed} | Skipped: ${totalSkipped}`,
    );
