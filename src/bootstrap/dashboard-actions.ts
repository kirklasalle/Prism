...
export function createDashboardActions(
    ...
    return [
        ...
        },
        {
            ...
            description:
                "Visual UI tour + a 2-step DAG + a real (best-effort) browser-use and computer-use probe so the operator can watch PRISM operate itself.",
            run: () =>
                guarded(async () => {
                    const dag = workflowExecutor.createDAG(
                        ...
                        [
                            ...
                            },
                            {
                                ...
                                args: { mode: "episodic_recent", limit: 2, sessionId },
                                risk: "low",
                                mutatesState: false,
                            },
                        ],
                    ...
                    const tour = svc
                        ? svc.broadcastUiTour([
                              ...
                              {
                                  tabId: "agentic",
                                  anchor: "guardian-status",
                                  dwellMs: 1500,
                                  message: "Guardian observing the run",
                              },
                              {
                                  tabId: "computer",
                                  dwellMs: 1500,
                                  message: "Computer-use probe — capturing a screengrab",
                              },
                              {
                                  tabId: "browser",
                                  dwellMs: 1800,
                                  message: "Browser-use probe — launching a headless session",
                              },
                              { tabId: "logs", anchor: "actions", dwellMs: 1500, message: "Quick Actions running" },
                              {
                                  tabId: "logs",
                                  anchor: "action-history",
                                  dwellMs: 1500,
                                  message: "Action recorded in history",
                              },
                    ...
                    const cuaProbe = (async () => {
                        ...
                        try {
                            ...
                            activityBus.emit({
                                ...
                                operation: "cua.screengrab",
                                status: "succeeded",
                                details: {
                                    source: "workflow_demo",
                                    path: (result as Record<string, unknown>)?.path ?? null,
                                },
                            });
                        } catch (err) {
                            activityBus.emit({
                    ...
                    // BUA/CUA probes.
                    await Promise.all([orchestrator.runWorkflow(dag), tour, cuaProbe, buaProbe]);
