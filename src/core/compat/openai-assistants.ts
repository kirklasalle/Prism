...
export type ShimErrorCode = "invalid_request" | "not_found" | "missing_field" | "executor_failed";
...
export async function handleChatCompletions(
    ...
    for (const m of req.messages) {
        ...
        if (typeof m.role !== "string" || typeof m.content !== "string") {
            throw new ShimError("invalid_request", "every message must have string 'role' and 'content'");
        }
    }
    if (req.stream === true) {
        throw new ShimError(
            "invalid_request",
            "streaming is not yet supported by the PRISM OpenAI compat shim; " + "use POST /api/chat/stream for SSE",
        );
    }
    ...
    try {
        ...
        throw new ShimError("executor_failed", `chat executor failed: ${(err as Error).message}`);
    }
...
/** POST /v1/threads */
export function handleCreateThread(req: CreateThreadRequest | undefined, deps: ShimHandlerDeps): ThreadObject {
    const thread = deps.store.createThread(req?.metadata ?? {});
    if (req?.messages) {
        for (const m of req.messages) {
            if (m.role !== "user" || typeof m.content !== "string") {
                throw new ShimError("invalid_request", "thread seed messages must have role='user' and string content");
            }
...
/** POST /v1/threads/:id/messages */
export function handleCreateMessage(threadId: string, req: CreateMessageRequest, deps: ShimHandlerDeps): ThreadMessage {
    if (!req || req.role !== "user" || typeof req.content !== "string") {
        throw new ShimError("invalid_request", "messages.create requires role='user' and string content");
    }
...
export async function handleCreateRun(
    ...
    if (messages.length === 0) {
        throw new ShimError("invalid_request", "cannot run an empty thread; create at least one message first");
    }
...
/** GET /v1/threads/:id/runs/:run_id */
export function handleGetRun(threadId: string, runId: string, deps: ShimHandlerDeps): RunObject {
