...
export class DevIdentityProvider {
    ...
    constructor(workspaceStateDir: string, runtimeSessionId: string, activityBus?: ActivityBus) {
    ...
    private persistIdentity(): void {
        ...
        try {
            ...
            writeFileSync(join(this.stateDir, STATE_FILENAME), JSON.stringify(snapshot, null, 2) + "\n", {
                encoding: "utf-8",
            });
        } catch {
    ...
    private loadPersistedIdentity(): DevIdentitySnapshot | null {
        try {
            ...
            if (!parsed.operator?.operatorId || !parsed.operator?.email || !parsed.agent?.agentId) {
                return null;
            }
    ...
    private emitEvent(operation: string, status: "succeeded" | "failed", details: Record<string, unknown>): void {
