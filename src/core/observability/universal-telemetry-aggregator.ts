...
function generateSummary(operation: string, status: string, details: Record<string, unknown>): string {
    ...
    const readableOp = operation.replace(/\./g, " → ").replace(/_/g, " ");
...
function suggestAction(severity: TelemetrySeverity, operation: string): string | null {
    ...
    if (severity === "error" && operation.includes("browser"))
        return "Check browser session health and Playwright availability.";
    if (severity === "error" && operation.includes("computer"))
        return "Verify command execution permissions and system access.";
...
export class UniversalTelemetryAggregator implements ActivitySubscriber {
    ...
    ingestConsoleLine(line: ConsoleLine): UnifiedTelemetryEntry {
        const severity =
            line.stream === "stderr" ? classifySeverityFromText(line.line) : classifySeverityFromText(line.line);
    ...
    ingestRaw(
        partial: Partial<UnifiedTelemetryEntry> & {
            operation: string;
            source: string;
            summary: string;
        },
