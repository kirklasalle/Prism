...
export function bootstrapUtilities(options: UtilitiesBootstrapOptions): UtilitiesConfig {
    ...
    const riskOverrideStore = new RiskOverrideStore(workspacePath("state", "risk-overrides.json"), activityBus);
    ...
    registerBuiltInUtilities(utilityRegistry, {
        ...
        runRetrievalTrends: async () => {
            ...
            return {
                summary: report ? `Trend report ready (${report.snapshotsCompared} snapshots).` : "No trend data yet.",
                details: { report },
            };
        },
        runPerfTrendReport: async () => {
            ...
            return {
                summary:
                    out.code === 0 ? "Perf trend report generated." : `Perf trend report failed (exit ${out.code}).`,
                details: { exitCode: out.code, stdout: out.stdout.slice(-2000), stderr: out.stderr.slice(-2000) },
            };
        },
    });
