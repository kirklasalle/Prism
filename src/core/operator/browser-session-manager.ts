...
export type BrowserSessionState = "idle" | "launching" | "active" | "navigating" | "suspended" | "terminated";
...
export class BrowserSessionManager {
    ...
    async launch(options?: BrowserLaunchOptions): Promise<BrowserSession> {
        ...
        try {
            ...
            if (vp) {
                meta.viewportWidth = vp.width;
                meta.viewportHeight = vp.height;
            }
        } catch {
            /* not critical */
        }
    ...
    async closeSession(sessionId: string): Promise<void> {
        ...
        try {
            await Promise.race([(s.browser as any).close(), new Promise<void>((resolve) => setTimeout(resolve, 5000))]);
        } catch {
            /* already closed */
        }
