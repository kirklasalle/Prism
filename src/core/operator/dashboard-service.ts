...
function computeWindowMetrics(
    ...
    const label = (
        windowMs === TELEMETRY_WINDOW_MS["1h"] ? "1h" : windowMs === TELEMETRY_WINDOW_MS["1d"] ? "1d" : "7d"
    ) as TelemetryWindow;
    return {
        windowLabel: label,
        windowMs,
...
function computeTelemetrySummary(events: readonly ActivityEvent[], windowLabel: TelemetryWindow): TelemetrySummary {
    const now = Date.now();
...
function computeSloSummary(store: MetricsStore): SloSummary {
    ...
    const metrics: SloMetric[] = SLO_TARGETS.map(({ histName, label, targetP95Ms, targetP99Ms }) => {
        ...
        const merged: HistogramSnapshot = {
            name: histName,
            labels: {},
            buckets,
            counts: mergedCounts,
        ...
        const p50Ms = histogramPercentile(merged, 0.5);
...
function eventCorrelationId(event: ActivityEvent): string | null {
    ...
    return typeof correlationId === "string" && correlationId.trim().length > 0 ? correlationId : null;
}
...
function buildCorrelatedTraceSummaries(
    ...
    for (const event of events) {
        ...
        const details = event.details as Record<string, unknown> | undefined;
        const eventChatSessionId = typeof details?.chatSessionId === "string" ? details.chatSessionId : null;
    ...
    return Array.from(grouped.entries())
        .map(([correlationId, traceEvents]) => {
            ...
            const firstChatSessionId =
                typeof firstDetails?.chatSessionId === "string" ? firstDetails.chatSessionId : null;
            return {
                correlationId,
...
function computeRuntimeExcellenceSnapshot(
    ...
    const approvalFailures = recentEvents.filter(
    ...
    const runtimeHealthRaw =
        100 -
        failureRate * 60 -
        Math.min(25, approvalFailures * 8) -
        traceFailureRate * 20 -
        Math.min(20, retrievalAlertCount * 4);
    const memoryConfidenceRaw =
        100 - Math.min(60, retrievalAlertCount * 12) - Math.min(20, Math.round(failureRate * 40));
...
export class DashboardService {
    ...
    private telemetryAggregator:
        | import("../observability/universal-telemetry-aggregator.js").UniversalTelemetryAggregator
        | null = null;
    ...
    private async handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
        ...
        if (method === "POST" && url === "/api/system/shutdown") {
            ...
            res.end(
                JSON.stringify({
                    success: true,
                    message: "Graceful shutdown sequence initialized by operator console.",
                }),
            );
        ...
        if (method === "POST" && url === "/api/models/recommended") {
            try {
                const body = await this.readJsonBody<{ name: string; fileName: string; path: string; source: string }>(
                    req,
                );
            ...
            try {
                ...
                if (ckId === "local-llm-service-ready") {
                    ...
                    const activeProviderId =
                        activeSession?.llmProviderId ?? (await this.llmProviders.getCatalog()).activeProviderId ?? null;

                    if (activeProviderId === "llamacpp" || activeProviderId === "bitnetcpp") {
                        const supervisor =
                            activeProviderId === "llamacpp" ? this.llamaSupervisor : this.bitnetSupervisor;
                        if (supervisor) {
                            ...
                            fixed = true;
                            detail =
                                "Cleared errored local LLM service slots. Dynamic on-demand loading will re-attempt on next chat.";
                        }
                    } else {
        ...
        if (method === "GET" && url === "/api/setup/status") {
            ...
            return this.json(res, 200, {
                ...
                executionProfileSegment:
                    prefs?.executionProfileSegment ?? this.status.executionProfileSegment ?? "individual",
        ...
        if (method === "POST" && url === "/api/setup/cac") {
            try {
                ...
                const browserTool = this.tools.find((t) => t.name === "browser_control") as
                    | BrowserControlTool
                    | undefined;
                if (browserTool) {
                    try {
        ...
        if (method === "POST" && url === "/api/setup/initialization-session") {
            try {
                ...
                this.chatStore.appendMessage(session.sessionId, "assistant", certContent, {
                    source: "initialization_certificate",
                    type: "certificate",
                });
        ...
        if (method === "GET" && url.startsWith("/api/telemetry/unified")) {
            ...
            try {
                ...
                return this.json(res, 200, {
                    entries: this.telemetryAggregator.getTail(100),
                    stats: this.telemetryAggregator.getStats(),
                });
            }
        }
        ...
        if (method === "POST" && url === "/api/telemetry/auth-trace") {
            try {
                ...
                return this.json(res, 400, { error: "Invalid auth trace payload" });
            }
        }
                    ...
                    if (pkg) {
                        ...
                        if (!pkg.sessionIds.some((sid) => operatorSessionIds.has(sid))) {
                            return this.json(res, 403, {
                                error: "forbidden",
                                message: "You do not have access to this session package.",
                            });
                        }
                    }
                }
        ...
        if (sessionPackageMatch && method === "DELETE") {
            try {
                ...
                return this.json(
                    res,
                    200,
                    this.deleteSessionPackage(packageId, req.headers["x-prism-source"]?.toString() || "dashboard_api"),
                );
            } catch (error) {
                ...
                const candidates = events
                    .filter((e) => {
                        ...
                        return (
                            ...
                            (e.operation.includes("llm") || e.operation.startsWith("dashboard.llm"))
                        );
                    })
                    .slice(-limit)
        ...
        if (method === "POST" && url === "/api/hardware/swarm/load") {
            try {
                ...
                if (!modelPath) {
                    if (
                        body.model &&
                        (body.model.endsWith(".gguf") || body.model.includes("/") || body.model.includes("\\"))
                    ) {
                        ...
                        return this.json(res, 400, {
                            error: `Model "${modelAlias}" not found in local models directory.`,
                        });
                    }
                }
            ...
            try {
                ...
                if (!status.modelPath) {
                    return this.json(res, 400, {
                        ...
                        suggestion:
                            "Please select a GGUF model from the dropdown in the Guardian panel before starting.",
                    });
                }
                ...
                for (const provider of enabledProviders) {
                    try {
                        ...
                        results.push({
                            ...
                            suggested: disc.suggested.length,
                        });
                    } catch {
        ...
        if (pluginToggleMatch && method === "POST") {
            ...
            try {
                body = await this.readJsonBody<{ enabled?: boolean }>(req);
            } catch {
                /* no body — flip */
            }
            const newEnabled =
                typeof body.enabled === "boolean" ? body.enabled : !this.pluginStates[pluginName].enabled;
        ...
        if (method === "POST" && url === "/api/plugins/install") {
            ...
            if (body.manifest) {
                ...
                const profile = (prefs?.executionProfileSegment === "business" ? "business" : "individual") as
                    | "individual"
                    | "business";
                const result = loadPluginPack(body.manifest, body.packPath ?? ".", this.activityBus, {
        ...
        if (method === "GET" && url === "/api/agents") {
            ...
            const telemetry = this.agentTelemetry?.getGlobalStats() ?? {
                activeAgents: 0,
                tasksCompleted: 0,
                tasksFailed: 0,
                avgResponseMs: 0,
        ...
        if (method === "POST" && url === "/api/agents/launch") {
            ...
            const instance = this.agentLifecycle.spawn({
                ...
                modelOverride:
                    body.providerId && body.model ? { providerId: body.providerId, model: body.model } : undefined,
            });
            // Sync model override to LLM provider routing
            if (instance.modelOverride) {
                this.llmProviders.setAgentModelOverride(
                    instance.agentId,
                    instance.modelOverride.providerId,
                    instance.modelOverride.model,
                );
            }
            // Register in agent pool
            this.agentPool?.register({
        ...
        if (method === "GET" && url === "/api/agents/telemetry") {
            ...
            const recommendations =
                this.agentLifecycle && this.agentTelemetry
                    ? this.agentTelemetry.getPromotionRecommendations(this.agentLifecycle)
                    : [];
            const global = this.agentTelemetry?.getGlobalStats() ?? {
                activeAgents: 0,
                tasksCompleted: 0,
                tasksFailed: 0,
                avgResponseMs: 0,
        ...
        if (method === "GET" && url === "/api/computer/system-info") {
            ...
            try {
                ...
                try {
                    const { execSync } = await import("node:child_process");
                    const wmicOut = execSync("wmic path Win32_VideoController get Name,AdapterRAM /format:csv", {
                        timeout: 5000,
                        encoding: "utf8",
                    });
                    const lines = wmicOut
                    ...
                    if (lines.length > 0) {
                        ...
                        if (cols.length >= 3) {
                            ...
                            gpu = {
                                name: cols[2]?.trim() || "Unknown GPU",
                                vramTotalMb: Math.round(adapterRam / 1048576),
                                driverVersion: "",
                                cudaVersion: "",
                            };
                        }
        ...
        if (method === "GET" && url === "/api/computer/usage") {
            ...
            try {
                const { execSync } = await import("node:child_process");
                const nvOut = execSync(
                    "nvidia-smi --query-gpu=memory.used,memory.total,utilization.gpu,utilization.memory,temperature.gpu --format=csv,noheader,nounits",
                ...
                if (parts.length >= 5) {
                    gpuUsage = {
                        vramUsedMb: parseInt(parts[0], 10) || 0,
                        vramTotalMb: parseInt(parts[1], 10) || 0,
                        gpuUtilPct: parseInt(parts[2], 10) || 0,
                        memUtilPct: parseInt(parts[3], 10) || 0,
        ...
        if (method === "GET" && url === "/api/computer/screengrab/latest") {
            ...
            try {
                ...
                return this.json(res, 500, { error: "Failed to read latest screengrab" });
            }
        }
        ...
        if (method === "GET" && url === "/api/computer/screengrab/diagnostics") {
            ...
            if (dirExists) {
                ...
                checks.push({
                    ...
                    detail: `${allFiles.length} PNG file(s) in framebuffer directory`,
                });
                checks.push({
                    name: "Latest frame",
                    ok: latestExists,
                    detail: latestExists ? "latest.png present" : "No latest.png \u2014 capture has not run yet",
                });
            }
        ...
        if (method === "GET" && url?.startsWith("/api/computer/screengrab/file/")) {
            ...
            try {
                ...
                return this.json(res, 500, { error: "Failed to read file" });
            }
        }
...
`;
            try {
                ...
                const devices: Record<
                    string,
                    Array<{ name: string; status: string; props: Record<string, string> }>
                > = {};
                ...
                > = {
                    ...
                    "Network Adapters": Object.entries(nets).map(([name, addrs]) => ({
                        name,
                        status: "OK",
                        props: { addresses: (addrs || []).map((a) => a.address).join(", ") },
                    })),
                    "Display Adapters": [],
                    "Disk Drives": [],
                };
        ...
        if (method === "GET" && url.startsWith("/api/computer/devices/properties/")) {
            ...
            try {
                ...
                const r2 = await execAsync2(`powershell -NoProfile -NonInteractive -Command "${ps2}"`, {
                    timeout: 15000,
                    maxBuffer: 512 * 1024,
                });
        ...
        if (method === "POST" && url === "/api/computer/devices/report") {
            ...
            const lines: string[] = [
                "PRISM Device Manager — Hardware Report",
            ...
            try {
                ...
                for (const cat of cats) {
                    ...
                    try {
                        ...
                        const r3 = await execAsync3(`powershell -NoProfile -NonInteractive -Command "${ps3}"`, {
                            timeout: 15000,
                            maxBuffer: 1024 * 1024,
                        });
            ...
            try {
                ...
                const outDir =
                    ...
                    (process.env.PRISM_DATA_DIR
                        ? pathJoin(process.env.PRISM_DATA_DIR, "ptac")
                        : pathJoin(process.cwd(), "prism-output", "ptac"));
                let entries: string[] = [];
                try {
        ...
        if (method === "POST" && url === "/api/ptac/demo/run") {
            ...
            try {
                ...
                const args = [cliPath, "--profile=sandbox", `--suite=${suite}`, "--demo-recording", "--record-video"];
                    ...
                    if (!gotStdoutComplete) {
                        for (const ws of this.wsClients) {
                            try {
                                ws.send(
                                    JSON.stringify({
                                        type: "computer_diagnostics_complete",
                                        timestamp: new Date().toISOString(),
                                    }),
                                );
                            } catch {
            ...
            child.stderr!.on("data", (chunk: Buffer) => {
                try {
                    ...
                    for (const line of lines) {
                        if (stderrNoiseRe.test(line)) continue;
                        const msg = {
                            type: "knowledge_graph_diagnostics_log",
                            source: "stderr",
                            message: line.slice(0, 1024),
                            timestamp: new Date().toISOString(),
                        };
            ...
            child.stderr!.on("data", (chunk: Buffer) => {
                try {
                    ...
                    for (const line of lines) {
                        if (stderrNoiseRe.test(line)) continue;
                        const msg = {
                            type: "workspace_diagnostics_log",
                            source: "stderr",
                            message: line.slice(0, 1024),
                            timestamp: new Date().toISOString(),
                        };
        ...
        if (method === "POST" && url === "/api/network/vrgc/performance") {
            try {
                ...
                return this.json(res, 500, {
                    ok: false,
                    error: (err as Error).message ?? "VRGC performance test failed",
                });
            }
        }
            ...
            child.stderr!.on("data", (chunk: Buffer) => {
                try {
                    ...
                    for (const line of lines) {
                        if (stderrNoiseRe.test(line)) continue;
                        const msg = {
                            type: "telemetry_diagnostics_log",
                            source: "stderr",
                            message: line.slice(0, 1024),
                            timestamp: new Date().toISOString(),
                        };
            ...
            child.stderr!.on("data", (chunk: Buffer) => {
                try {
                    ...
                    for (const line of lines) {
                        if (stderrNoiseRe.test(line)) continue;
                        const msg = {
                            type: "scheduler_diagnostics_log",
                            source: "stderr",
                            message: line.slice(0, 1024),
                            timestamp: new Date().toISOString(),
                        };
                    ...
                    if (!gotStdoutComplete) {
                        for (const ws of this.wsClients) {
                            try {
                                ws.send(
                                    JSON.stringify({
                                        type: "scheduler_diagnostics_complete",
                                        timestamp: new Date().toISOString(),
                                    }),
                                );
                            } catch {
        ...
        if (method === "GET" && url === "/api/retrieval/alerts") {
            ...
            try {
                ...
                const allAlerts = [...diagnostics.alerts, ...cohortDashboard.alerts];
        ...
        if (method === "GET" && url.startsWith("/api/retrieval/prioritized-alerts")) {
            ...
            try {
                ...
                const prioritized = buildPrioritizedAlerts(rawAlerts, "retrieval", now).sort(
        ...
        if (method === "POST" && actionMatch) {
            ...
            try {
                const payload = await this.readJsonBody<{ sessionId?: string }>(req).catch(() => ({
                    sessionId: undefined,
                }));
        ...
        if (method === "POST" && /^\/api\/utilities\/[^/]+\/execute$/.test(url)) {
            ...
            try {
                const body = await this.readJsonBody<{ params?: Record<string, unknown>; reason?: string }>(req).catch(
                    () => ({}) as { params?: Record<string, unknown>; reason?: string },
                );
                const params =
                    body && "params" in body ? (body as { params?: Record<string, unknown> }).params : undefined;
            ...
            try {
                ...
                if (!provider || !email) {
                    return this.json(res, 400, {
                        ...
                        required: ["provider", "verifiedEmail"],
                    });
                }
                const updated = this.characterAccountabilityManager.markEmailVerified(assignmentId, email, provider);
                if (!updated)
                    return this.json(res, 409, {
                        error: "Verification rejected (assignment missing/revoked or email mismatch)",
                    });
        ...
        if (method === "GET" && url.startsWith("/api/retrieval/incident-trends")) {
            ...
            const report = this.incidentTrendStore.getReport(
                profile as "individual" | "business" | "unknown",
                windowDays,
            );
        ...
        if (url.startsWith("/api/incubation/")) {
            ...
            if (method === "POST" && url === "/api/incubation/ccc/compile") {
                ...
                const { INDIVIDUAL_PROFILE: ind, BUSINESS_PROFILE: biz } =
                    await import("../policy/execution-profiles.js");
            ...
            if (method === "POST" && url === "/api/incubation/shws/propose") {
                ...
                const candidate = inc.synthesizer.proposeFallback({
                    ...
                    dag: {
                        ...
                        name: body.dag.name ?? "ad-hoc",
                        steps: body.dag.steps as Array<import("../runtime/workflow.js").WorkflowStep>,
                        fallbacks: (body.dag.fallbacks ?? []) as Array<
                            import("../runtime/workflow.js").WorkflowFallback
                        >,
                    },
                    profile,
        ...
        // ── LLRE Efficacy Toggle preference ──────────────────────────────────────────
        if (method === "POST" && url === "/api/preferences/llre-efficacy") {
            ...
            this.json(res, 200, { updated: true, llreEnabled: enabled });
            return;
        }
        ...
        if (method === "GET" && url.startsWith("/api/auth/gmail/callback")) {
            ...
            this.serveOAuthPopupResult(res, "gmail", !!result.connected);
            return;
        }
        ...
        if (method === "GET" && url.startsWith("/api/auth/outlook/callback")) {
            ...
            this.serveOAuthPopupResult(res, "outlook", !!result.connected);
            return;
        }
