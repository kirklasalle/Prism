...
export class AgenticHandler implements IRouteHandler {
    match(req: IncomingMessage): boolean {
        ...
        if (pathname.startsWith("/api/actions/") || pathname === "/api/actions" || pathname === "/api/action-history")
            return true;
        ...
        if (
            ...
            (url.startsWith("/approve/") ||
                url.startsWith("/api/approve/") ||
                url.startsWith("/deny/") ||
                url.startsWith("/api/deny/"))
        )
            return true;

        if (method === "GET" && (url === "/pending" || url === "/api/pending")) return true;
    ...
    async handle(req: IncomingMessage, res: ServerResponse, service: DashboardService): Promise<void> {
            ...
            try {
                ...
                if (ckId === "local-llm-service-ready") {
                    ...
                    const activeProviderId =
                        activeSession?.llmProviderId ??
                        (await service.getLlmProviders().getCatalog()).activeProviderId ??
                        null;

                    if (activeProviderId === "llamacpp" || activeProviderId === "bitnetcpp") {
                        const supervisor =
                            activeProviderId === "llamacpp"
                                ? service.getLlamaSupervisor()
                                : service.getBitnetSupervisor();
                        if (supervisor) {
        ...
        if (method === "POST" && actionMatch) {
            ...
            try {
                const payload = await service
                    .readJsonBody<{ sessionId?: string }>(req)
        ...
        if (method === "GET" && url === "/api/agents") {
            ...
            const telemetry = agentTelemetry?.getGlobalStats() ?? {
                activeAgents: 0,
                tasksCompleted: 0,
                tasksFailed: 0,
                avgResponseMs: 0,
        ...
        if (method === "GET" && url === "/api/agents/telemetry") {
            ...
            const recommendations =
                agentLifecycle && agentTelemetry ? agentTelemetry.getPromotionRecommendations(agentLifecycle) : [];
            const global = agentTelemetry?.getGlobalStats() ?? {
                activeAgents: 0,
                tasksCompleted: 0,
                tasksFailed: 0,
                avgResponseMs: 0,
