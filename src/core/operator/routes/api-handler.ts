...
export class ApiHandler implements IRouteHandler {
    ...
    async handle(req: IncomingMessage, res: ServerResponse, service: DashboardService): Promise<void> {
        ...
        if (method === "GET" && url === "/api/health/extended") {
            ...
            const uptimeS = Number.isFinite(startedAtMs)
        ...
        if (method === "POST" && url === "/api/mode") {
            ...
            req.on("end", async () => {
                try {
                    ...
                    console.log(
                        `[PRISM][paradigm] Paradigm dynamically switched by operator. baseMode = ${targetBaseMode} (auto = ${isAuto})`,
                    );
            ...
            try {
                ...
                if (!snapshot) {
                    try {
                        ...
                        try {
                            const wmicOut = execSync("wmic path Win32_VideoController get AdapterRAM /format:csv", {
                                timeout: 3000,
                                encoding: "utf8",
                            });
                            const lines = wmicOut
                ...
                if (guardianAgent) {
                    ...
                    if (
                        gStatus &&
                        (gStatus.state === "running" || gStatus.state === "healing" || gStatus.state === "starting") &&
                        gStatus.modelPath
                    ) {
                        ...
                        if (!exists) {
                            try {
                                ...
                                const isGpu =
                                    gStatus.slotInfo?.gpuLayers === undefined ||
                                    gStatus.slotInfo.gpuLayers === null ||
                                    gStatus.slotInfo.gpuLayers > 0;
                                const vramBytes = isGpu ? sizeBytes : 0;
                                loadedModels.push({
                                    name: alias,
                                    sizeBytes,
