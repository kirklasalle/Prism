...
export class AutonomousHandler implements IRouteHandler {
    ...
    async handle(req: IncomingMessage, res: ServerResponse, service: DashboardService): Promise<void> {
        ...
        const url = normalizedUrl.startsWith("/api/v1/")
            ? "/api/" + normalizedUrl.substring("/api/v1/".length)
            : normalizedUrl;
        ...
        if (method === "POST" && (url === "/api/autonomous/goals" || url === "/api/autonomous/goal")) {
            ...
            const allowBrowser =
                body.allowBrowserUse ?? bodyConstraints.allowBrowserUse ?? bodyConstraints.allowBrowser;
            if (allowBrowser != null) constraints.allowBrowserUse = Boolean(allowBrowser);

            const allowComputer =
                body.allowComputerUse ?? bodyConstraints.allowComputerUse ?? bodyConstraints.allowComputer;
            ...
            if (requireApproval != null)
                constraints.requireApprovalAboveRisk = requireApproval as "low" | "medium" | "high";
        ...
        if (
            method === "GET" &&
            (url === "/api/autonomous/goals" ||
                url === "/api/autonomous/history" ||
                url.startsWith("/api/autonomous/history"))
        ) {
