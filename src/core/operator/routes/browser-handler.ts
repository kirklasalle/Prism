...
import { SSHP_SENSITIVE_SELECTORS } from "../sshp-interceptor.js";
    ...
    async handle(req: IncomingMessage, res: ServerResponse, service: DashboardService): Promise<void> {
        ...
        if (method === "GET" && url === "/api/browser/sessions") {
            ...
            return this.json(res, 200, {
                sessions: sessions.map((s: any) => ({ ...s, sessionId: s.id }) as Record<string, unknown>),
            });
        }
        ...
        if (method === "POST" && url === "/api/browser/profiles") {
            ...
            try {
                ...
                const activeAssignments = service.getCharacterAccountabilityManager().list({ state: "active" });
                const active = activeAssignments[0];
                let email = (body.email || body.prismUserEmail || "").trim();
                let assignmentId = body.assignmentId;
                if (active) {
                    if (!email) email = active.prismUserEmail;
                    if (!assignmentId) assignmentId = active.assignmentId;
                }
                const segment = (
                    body.segment ||
                    body.executionProfileSegment ||
                    (active?.executionProfileSegment as string) ||
                    "individual"
                ).trim();
        ...
        if (method === "POST" && url === "/api/browser/launch") {
            ...
            try {
                ...
                const activeAssignments = service.getCharacterAccountabilityManager().list({ state: "active" });
                const active = activeAssignments[0];
                const launchOptions = { ...body };
                if (active) {
                    if (!launchOptions.assignmentId) launchOptions.assignmentId = active.assignmentId;
                    if (!launchOptions.prismUserEmail) launchOptions.prismUserEmail = active.prismUserEmail;
                    if (!launchOptions.characterId) launchOptions.characterId = active.characterId;
                }
                const session = await mgr.launch(launchOptions);
            ...
            try {
                ...
                if (service.sshpInterceptor.isEnabled()) {
                    ...
                    if (handles && handles.page) {
                        const rects = await handles.page.evaluate(
                            (selectors: string[]) => {
        ...
        if (networkMatch && method === "GET") {
            ...
            const logs = mgr.getNetworkLog(sessionId);
            return this.json(res, 200, { entries: logs, log: logs });
        }
        ...
        if (domMatch && method === "GET") {
            ...
            try {
                ...
                return this.json(res, 200, { html, dom: html });
            } catch (err) {
                return this.json(res, 500, { error: String(err) });
            }
        }
