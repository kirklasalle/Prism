    ...
    async handle(req: IncomingMessage, res: ServerResponse, service: DashboardService): Promise<void> {
            ...
            if (extractedSessionId) {
                ...
                if (session) {
                    ...
                    if (!hasOwnership) {
                        ...
                        res.end(
                            JSON.stringify({ error: "forbidden", message: "You do not have access to this session." }),
                        );
                        return;
                    }
                }
        ...
        if (sessionCharacterMatch && method === "POST") {
            try {
                ...
                if (!cacAssignmentId) {
                    const rawEmail =
                        principal && principal.email ? principal.email : (body.operatorEmail ?? `operator@prism.local`);
                    ...
                    if (!isAdmin && principal) {
                        const assignmentInfo = service
                            .getCharacterAccountabilityManager()
                            .getAssignmentChain(cacAssignmentId);
        ...
        if (method === "GET" && url.startsWith("/api/auth/gmail/callback")) {
            ...
            return this.html(res, "gmail", !!result.connected);
        }
        ...
        if (method === "GET" && url.startsWith("/api/auth/outlook/callback")) {
            ...
            return this.html(res, "outlook", !!result.connected);
        }
