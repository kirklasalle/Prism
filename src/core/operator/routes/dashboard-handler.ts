...
export class DashboardHandler implements IRouteHandler {
    ...
    async handle(req: IncomingMessage, res: ServerResponse, service: DashboardService): Promise<void> {
        ...
        if (!principal && isTokenValid) {
            const adminUser = store.getUserByEmail("default", "admin@prismrefraction.com");
            if (adminUser) {
        ...
        const useSimple =
            isExplicitSimple ||
            (!isExplicitDashboard && (prefs?.uiMode === "simple" || (!prefs?.uiMode && sessionCount === 0)));
        ...
        res.end(
            useSimple ? simpleModeHtml(service.getPort(), clientToken) : dashboardHtml(service.getPort(), clientToken),
        );
    }
}
