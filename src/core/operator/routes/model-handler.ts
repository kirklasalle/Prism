 ...
 * Served from the backend so it can be updated without a frontend deploy and
 * is immune to the charset double-encoding corruption that afflicted the
 * previous frontend constant.
 */
const BUILTIN_RECOMMENDED_CATALOG: Array<{
    name: string;
    fileName: string;
    size: string;
    url: string;
    ...
    async handle(req: IncomingMessage, res: ServerResponse, service: DashboardService): Promise<void> {
        ...
        if (method === "DELETE" && url === "/api/models/recommended") {
            try {
                ...
                service.customRecommendedModels = service.customRecommendedModels.filter(
                    (m) => m.fileName !== body.fileName,
                );
                service.saveCustomRecommendedModels();
                return this.json(res, 200, { custom: service.customRecommendedModels });
            } catch (error) {
