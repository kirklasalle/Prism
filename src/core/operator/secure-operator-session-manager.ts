...
export interface SecureOperatorSessionManager {
    ...
    createSession(authRequest: CacAuthRequest, options: SecureOperatorSessionOptions): Promise<SecureOperatorSession>;
...
export class DefaultSecureOperatorSessionManager extends EventEmitter implements SecureOperatorSessionManager {
    ...
    isOperationAllowed(sessionId: string, operation: string): boolean {
        ...
        return (
            session.securityConstraints.allowedOperations.includes(operation) ||
        ...
        switch (session.sessionType) {
            ...
            case "full_control":
                ...
                const browserSession = await this.browserSessionManager.launch({
                    ...
                    assignmentId:
                        session.cacSession.cacAssignmentId || `cac_${session.cacSession.certificateInfo.cacId}`,
    ...
    private async bindToCharacterAccountability(session: SecureOperatorSession, characterId: string): Promise<void> {
    ...
    private getAllowedOperations(sessionType: SecureOperatorSessionType, privilege: CacOperatorPrivilege): string[] {
    ...
    private async emitActivity(operation: string, details: Record<string, unknown>, sessionId?: string): Promise<void> {
        const eventSessionId = sessionId ?? (typeof details.sessionId === "string" ? details.sessionId : "system");
