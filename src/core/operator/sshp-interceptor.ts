...
/**
 * Canonical list of CSS selectors that identify sensitive/PII-bearing DOM
 * elements whose visual bounding boxes should be masked during screenshot
 * redaction. Shared across browser-handler, browser-control-tool, and
 * autonomous-browser-agent so selector changes propagate everywhere.
 */
export const SSHP_SENSITIVE_SELECTORS: readonly string[] = [
    'input[type="password"]',
...
export class SSHPInterceptor {
     ...
     */
    async redactScreenshot(imageBuffer: Buffer, rects: SSHPRedactRect[]): Promise<Buffer> {
