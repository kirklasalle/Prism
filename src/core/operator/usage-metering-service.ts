...
export class UsageMeteringService {
    ...
    getCaps(): UsageCaps {
        const rows = this.db
            .prepare(`SELECT key, value FROM usage_caps WHERE key IN ('sessionCap','dailyCap','monthlyCap')`)
    ...
    checkCap(): CapCheckResult {
        ...
        if (caps.sessionCap !== null)
            candidates.push({ remaining: caps.sessionCap - sessionSpend, capType: "session" });
        if (caps.dailyCap !== null) candidates.push({ remaining: caps.dailyCap - dailySpend, capType: "daily" });
        if (caps.monthlyCap !== null)
            candidates.push({ remaining: caps.monthlyCap - monthlySpend, capType: "monthly" });
