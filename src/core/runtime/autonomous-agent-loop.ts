...
export class AutonomousAgentLoop {
    ...
    constructor(activityBus: ActivityBus, registry?: ToolRegistry, config?: Partial<AutonomousLoopConfig>) {
    ...
    async executeGoal(
        ...
        console.log(
            `[PRISM][AgentLoop] ▶ executeGoal called. GoalId: ${goalId}, Objective: "${goal.objective.slice(0, 100)}", Source: ${goal.source}, Constraints: browser=${goal.constraints.allowBrowserUse}, computer=${goal.constraints.allowComputerUse}, shell=${goal.constraints.allowShellExec}, maxActions=${goal.constraints.maxActions}`,
        );
        ...
        console.log(
            `[PRISM][AgentLoop] Goal ${goalId.slice(0, 8)} status → planning. Tools available: ${this.toolDefinitions.length}, generateFn bound: true`,
        );
        ...
        const isResearchObjective =
            /\b(find|search|look.?up|locate|browse|research|fetch|buy|purchase|listing|listings|price|deal|sale|car|vehicle|product|job|property|house|apartment|news|data)\b/.test(
                objectiveLower,
            );
        ...
        if (activeDefs.length < originalCount) {
            console.log(
                `[PRISM][AgentLoop] Tools pruned from ${originalCount} to ${activeDefs.length} based on objective relevance`,
            );
        }

        try {
            const result = await this.planner.executeGoal(goal, this, this.generateFn, activeDefs, {
    ...
    async executeStep(
        ...
        if (goal.totalActions >= goal.constraints.maxActions) {
            ...
            this.emit("autonomous.goal.budget_exhausted", "failed", {
                ...
                maxActions: goal.constraints.maxActions,
            });
        ...
        if (!this.isToolAllowed(toolName, goal.constraints)) {
            ...
            this.emit("autonomous.step.skipped", "succeeded", {
                ...
                stepId: step.stepId,
                tool: toolName,
                reason: "Tool not allowed by constraints",
            });
            return step;
        }
        ...
        try {
            console.log(
                `[PRISM][AgentLoop] Executing step: tool="${toolName}", iteration=${iteration}, args=${JSON.stringify(toolArgs)}`,
            );
                ...
                : {
                      ...
                      autonomousRecovery: {
                          ...
                          recommendedNextAction:
                              "Diagnose root cause, run rollback or compensating action, then continue toward the objective.",
                      },
                  };
            step.completedAt = new Date().toISOString();
            step.durationMs = Date.now() - Date.parse(step.startedAt!);
            console.log(
                `[PRISM][AgentLoop] Step execution completed: status=${step.status}, duration=${step.durationMs}ms`,
            );
            ...
            step.output = {
                ...
                autonomousRecovery: {
                    ...
                    continueObjective: true,
                    recommendedNextAction:
                        "Identify the failing assumption, execute rollback or compensating action, and continue with a revised step.",
                },
            };
            step.completedAt = new Date().toISOString();
    ...
    getActiveGoal(): AutonomousGoal | null {
        return this.activeGoalId ? (this.goals.get(this.activeGoalId) ?? null) : null;
    }
    ...
    getAABLedger(): AABLedgerEntry[] {
        return [...this.aabLedger];
    }
    isPaused(): boolean {
        return this.paused;
    }
    ...
    globalPause(): void {
        this.paused = true;
        this.emit("autonomous.global.paused", "succeeded", {});
    }
    globalResume(): void {
        this.paused = false;
        this.emit("autonomous.global.resumed", "succeeded", {});
    }
    ...
    private recordAAB(
        ...
        anomalyType: string,
        description: string,
        intervention: AABLedgerEntry["intervention"],
    ): void {
