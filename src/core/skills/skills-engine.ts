...
import {
    ...
    SkillSession,
    SkillStep,
    SkillSessionCreateOptions,
    SkillPermissionCheck,
    SkillAccountabilityChain,
    SkillExecutor,
} from "./types.js";
...
export class SkillsEngine {
    ...
    public async checkPermission(
        ...
        if (this.characterAccountability.assertSkillPermission) {
            return this.characterAccountability.assertSkillPermission(accountabilityChain.assignmentId, skillId);
        }
    ...
    public async createSession(
        ...
        const opts: SkillSessionCreateOptions =
            typeof skillIdOrOpts === "string"
        ...
        if (!permission.allowed) {
            throw new Error(`Skill '${opts.skillId}' denied: ${permission.reason}. ${permission.remediation ?? ""}`);
        }
    ...
    public async executeStep(sessionId: string): Promise<SkillSession> {
        ...
        const inputHash = crypto
            .createHash("sha256")
            .update(JSON.stringify({ stepId: step.id, state: session.statePayload }))
            .digest("hex");
        ...
        try {
            ...
            const mainPrompt = this.interpolate(skill.triad_templates.main_hemisphere, {
                ...
                const srResult = await this.providerManager.generateSR(
                    {
                        ...
                        conversation: [],
                        systemPrompt:
                            "You are the PRISM Spectrum Refraction orchestrator running a governed skill workflow.",
...
${mainPrompt}
    ...
    /**
     * Load and parse all skill definitions in the skills/ workspace directory recursively
     */
    public loadAllSkills(): void {
        ...
        try {
            const files = getAllFiles(this.skillsDir);
            for (const filePath of files) {
                if (filePath.endsWith(".json") || filePath.endsWith(".yaml") || filePath.endsWith(".yml")) {
                    try {
                        ...
                        console.error(`[SkillsEngine] Failed to parse skill file ${filePath}: ${err.message}`);
                    }
                }
            }
        } catch (err: any) {
            console.error(`[SkillsEngine] Failed to scan skills directory recursively: ${err.message}`);
        }
    }
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (currentSection === "governance") {
                    ...
                    if (govMatch) {
                        ...
                        else if (gk === "required_approvals")
                            result.governance.required_approvals = gv
                                .split(",")
                                .map((s) => s.trim())
                                .filter(Boolean);
                        else if (gk === "covenant_rules")
                            result.governance.covenant_rules = gv
                                .split(",")
                                .map((s) => s.trim())
                                .filter(Boolean);
                    }
                    ...
                    if (tmplMatch) {
                        ...
                        else if (tk === "right_hemisphere")
                            result.triad_templates.right_hemisphere = templateVal.trim();
