...
test("Missing manifest_version causes error", () => {
    ...
    assert(
        result.errors.some((e: any) => e.field === "manifest_version"),
        "Should have manifest_version error",
    );
});
...
test("Invalid pack_name format is rejected", () => {
    ...
    assert(!result.valid, "Manifest should be invalid");
    assert(
        result.errors.some((e: any) => e.field === "pack_name"),
...
test("Invalid semantic version is rejected", () => {
    ...
    assert(!result.valid, "Manifest should be invalid");
    assert(
        result.errors.some((e: any) => e.field === "pack_version"),
...
test("Missing license causes error", () => {
    ...
    assert(!result.valid, "Manifest should be invalid");
    assert(
        result.errors.some((e: any) => e.field === "license"),
        "Should have license error",
    );
});
...
test("Unsupported license is rejected", () => {
    ...
    assert(
        result.errors.some((e: any) => e.field === "license"),
        "Should reject unsupported license",
    );
});
...
test("Empty adapters array is rejected", () => {
    ...
    assert(!result.valid, "Manifest should be invalid");
    assert(
        result.errors.some((e: any) => e.field === "adapters"),
        "Should reject empty adapters",
    );
});
...
test("Invalid adapter_type is rejected", () => {
    ...
    assert(!result.valid, "Manifest should be invalid");
    assert(
        result.errors.some((e: any) => e.field.includes("adapter_type")),
...
test("Empty capabilities array is rejected", () => {
    ...
    assert(!result.valid, "Manifest should be invalid");
    assert(
        result.errors.some((e: any) => e.field.includes("capabilities")),
...
test("Missing tier_routing causes error", () => {
    ...
    assert(!result.valid, "Manifest should be invalid");
    assert(
        result.errors.some((e: any) => e.field.includes("tier_routing")),
        "Should reject missing tier_routing",
    );
});
...
test("Invalid default_tier is rejected", () => {
    ...
    assert(!result.valid, "Manifest should be invalid");
    assert(
        result.errors.some((e: any) => e.field.includes("default_tier")),
        "Should reject tier 5",
    );
});
...
test("Missing compatibility causes error", () => {
    ...
    assert(!result.valid, "Manifest should be invalid");
    assert(
        result.errors.some((e: any) => e.field === "compatibility"),
...
test("Empty profiles array is rejected", () => {
    ...
    assert(!result.valid, "Manifest should be invalid");
    assert(
        result.errors.some((e: any) => e.field === "compatibility.profiles"),
        "Should reject empty profiles",
    );
});
...
test("Unreviewed security status produces warning", () => {
    ...
    assert(result.valid, "Manifest should be valid");
    assert(
        result.warnings.some((w: any) => w.field === "security.review_status"),
        "Should warn about unreviewed status",
    );
});
...
test("Very short description produces warning", () => {
    ...
    assert(result.valid, "Manifest should be valid");
    assert(
        result.warnings.some((w: any) => w.field === "description"),
...
test("verifyEd25519Signature returns false for malformed inputs", () => {
    const manifest = createValidManifest();
    assert(
        !verifyEd25519Signature(manifest, "not-base64!!!", "not-base64!!!"),
        "Invalid inputs should return false not throw",
    );
});
...
test("PluginPackValidator individual profile warns but allows unsigned plugin", () => {
    ...
    const criticalSignatureErrors = result.errors.filter(
        (e) => e.field === "security.signature" && e.severity === "critical",
    );
